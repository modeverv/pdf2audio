...
def generate_audio_to_memory_voicevox(args):
    """
    1つの文章をVoiceVox APIで音声データとしてメモリに生成する（並列処理用）
    完全I/Oフリー - WAVファイルをバイナリデータとして直接取得
    
    Args:
        args (tuple): (index, sentence, voicevox_url, speaker_id)
    
    Returns:
        tuple: (success, index, wav_bytes, error_message)
    """
    i, sentence, voicevox_url, speaker_id = args
    
    try:
        # Step 1: audio_query を生成（メモリ上）
        ...
        # WAVバイナリを直接取得（ディスクI/O = 0）
        wav_bytes = synthesis_response.content
        
        return (True, i, wav_bytes, None)
    
    except requests.exceptions.RequestException as e:
        return (False, i, None, f"HTTP Error: {str(e)}")
    except Exception as e:
        return (False, i, None, str(e))


def convert_to_audio_parallel_memory_voicevox(sentences, voicevox_url="http://127.0.0.1:50021", 
                                                speaker_id=1, max_workers=None):
    """
    文章リストをVoiceVox APIで音声データに並列変換（完全I/Oフリー）
    
    Args:
        sentences (list): 文章のリスト
        voicevox_url (str): VoiceVoxサーバーのURL
        speaker_id (int): 話者ID（1=ずんだもん（ノーマル）など）
    ...
    print(f"方式: 完全I/Oフリー（メモリ上でHTTP通信のみ）\n")
    
    # 並列処理用の引数リストを作成
    tasks = [(i, sentence, voicevox_url, speaker_id) for i, sentence in enumerate(sentences)]
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 全タスクを投入
        future_to_index = {executor.submit(generate_audio_to_memory_voicevox, task): task[0] 
                          for task in tasks}
...
def concatenate_wav_binary(wav_bytes_list, output_filename):
    ...
    try:
        ...
        # 再生時間を計算（VoiceVoxは24000Hzが多い）
        # サンプルレートは実際のWAVヘッダーから読み取るべきだが、ここでは24000Hzと仮定
        sample_rate = 24000
...
if __name__ == "__main__":
    ...
    if sentences:
        ...
        # 想定処理時間の計算
        estimated_time_serial = len(sentences) * 2  # 1文2秒と仮定（VoiceVox）
        ...
        # 音声データをバイナリとして並列生成（完全I/Oフリー）
        wav_bytes_list = convert_to_audio_parallel_memory_voicevox(
            sentences, 
